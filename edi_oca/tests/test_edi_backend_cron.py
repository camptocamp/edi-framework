...
class EDIBackendTestCronCase(EDIBackendCommonComponentRegistryTestCase):
    ...
    def test_exchange_delete_superseded_records(self):
        ...
        # all the older records should have been superseded by record1_3
        records = self.record1 + record1_1 + record1_2
        self.backend._check_output_exchange_sync()
        for record in records:
            self.assertEqual(record.edi_exchange_state, "superseded")
        self.assertEqual(record1_3.edi_exchange_state, "output_sent")
        self.backend._delete_superseded_records()
        for record in records:
